# Replication lab – Teney (2016)

**What is this?** A ready-to-run replication exercise for the workshop "Applied replication for data skills" (Open Research Conference, Newcastle University, 16 June 2026). You take the seat of "analyst no. 6" in a published *Nature* study (Aczel et al. 2026, the Multi100 project) and test the robustness of a real claim from Teney (2016) – using the same data, tools, and workflow the project used. The report you build (`index.qmd`) renders to a website you publish yourself; every task is a small, committable step that doubles as a git lesson.
...
2. **Clone it in Positron.** In Positron: **File → New Folder from Git…**, paste your new repo's URL, and pick a local folder. Sign in to GitHub through the **Accounts** menu (browser OAuth – no tokens needed). If you have never set your git identity, run once in the terminal:
   ```sh
...
Tick a box when you have done the thing – and tick it *with a commit* (edit this file, commit, push), so even the checklist teaches git. The ten boxes map to the workshop's learning objectives.
...
- [ ] **10. Locate yourself in the multiverse.** I posted my result to the Discussion thread and can place my analysis among the others. *(Task E + debrief)*
...
- **Results board (Discussion thread):** https://github.com/CodeMoreh/applied-replication/discussions – paste your `report_result()` line and your repo URL here.
